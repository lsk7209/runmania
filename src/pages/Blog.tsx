...
const fetchBlogListPosts = async (): Promise<BlogListQueryResult> => {
  try {
    ...
    if (!response.ok) throw new Error(`Blog list request failed: ${response.status}`);
    ...
    return {
      ...
      fallbackMessage: "라이브 게시글이 비어 있어 로컬 백업 콘텐츠를 표시 중입니다.",
    };
  } catch (error) {
    console.error("[Blog] Failed to load live post list:", error);
    return {
      posts: await loadLocalPosts(),
      fallbackMessage: "라이브 게시글을 불러오지 못해 로컬 백업 콘텐츠를 표시 중입니다.",
    };
  }
};
...
const fetchBlogPost = async (slug: string): Promise<BlogPostQueryResult> => {
  ...
  const fallbackPost = localPosts.find((candidate) => candidate.slug === slug) ?? null;
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Blog detail request failed: ${response.status}`);
    const data = await response.json();
    return { post: data ? mapDbPostToLocal(data) : null, fallbackMessage: null };
  } catch (error) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-16 pt-24">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        {fallbackMessage ? <DataFallbackNotice message={fallbackMessage} /> : null}
...
const BlogDetail = ({ slug }: { slug: string }) => {
  ...
  const seoTitle = post?.generationMeta?.metaTitle || (post ? `${post.title} | 런닝화매니아` : "블로그 | 런닝화매니아");
  const seoDescription = post?.generationMeta?.metaDescription || (post ? post.excerpt : "발 건강과 러닝화에 대한 깊이 있는 분석");
  ...
  if (!post) {
    return (
      <div className="flex min-h-screen items-center justify-center pt-14">
        <p className="text-muted-foreground">{fallbackMessage ?? "포스트를 찾을 수 없습니다."}</p>
      </div>
    );
  }
