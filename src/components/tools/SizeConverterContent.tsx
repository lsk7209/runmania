...
import { ArrowRight, RotateCcw, Footprints, ArrowLeftRight } from "lucide-react";
...
interface SizeRow {
  kr: number; usM: number; usW: number; eu: number; uk: number; mm: number;
}
...
const SizeConverterContent = () => {
  ...
  const handleReset = () => { setFromRegion("KR"); setInputVal("270"); };
  ...
  return (
    <div>
      ...
      <div className="mb-6 rounded-2xl border border-border bg-card p-6">
        <label className="mb-3 block text-xs font-medium text-muted-foreground">기준 사이즈 체계</label>
        <div className="mb-4 flex flex-wrap gap-2">
          {regions.map((r) => (
            <button key={r.key} onClick={() => { setFromRegion(r.key); setInputVal(""); }}
              className={`rounded-lg border px-3 py-2 text-sm font-medium transition-all ${fromRegion === r.key ? "border-primary bg-primary/10 text-primary" : "border-border bg-secondary text-muted-foreground hover:text-foreground"}`}>
              {r.short}
            </button>
          ))}
        </div>
        <label className="mb-2 block text-xs font-medium text-muted-foreground">{regions.find((r) => r.key === fromRegion)?.label} 사이즈 입력</label>
        <div className="flex items-center gap-3">
          <Input type="number" step={fromRegion === "KR" ? "5" : "0.5"} value={inputVal} onChange={(e) => setInputVal(e.target.value)} placeholder={fromRegion === "KR" ? "270" : "9"} className="w-28 border-border bg-secondary text-center text-lg font-mono" />
          <span className="text-sm text-muted-foreground">{fromRegion === "KR" ? "mm" : fromRegion}</span>
        </div>
        <div className="mt-3 flex flex-wrap gap-2">
          {quickSizes[fromRegion].map((sz) => (
            <button key={sz} onClick={() => setInputVal(sz.toString())} className="rounded-lg border border-border bg-secondary px-3 py-1.5 text-[11px] text-muted-foreground transition-colors hover:border-primary/30 hover:text-foreground">
              {sz}
            </button>
          ))}
        </div>
      ...
      {matchedRow && inputVal && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
          ...
          {regions.map((r) => {
            ...
            return (
              <div key={r.key} className={`flex items-center justify-between rounded-xl border p-4 ${isFrom ? "border-primary/30 bg-primary/5" : "border-border bg-card"}`}>
                <div className="flex items-center gap-3">
                  <span className={`flex h-10 min-w-[56px] items-center justify-center rounded-lg text-xs font-bold ${isFrom ? "bg-primary/10 text-primary" : "bg-secondary text-muted-foreground"}`}>{r.short}</span>
                  <p className="text-xs text-muted-foreground">{r.label}</p>
                </div>
                <span className={`font-mono text-sm font-bold ${isFrom ? "text-primary" : ""}`}>{formatVal(val, r.key)}</span>
              </div>
            );
          })}
          <div className="flex items-center justify-between rounded-xl border border-border bg-card p-4">
            <div className="flex items-center gap-3">
              <span className="flex h-10 min-w-[56px] items-center justify-center rounded-lg bg-secondary text-xs font-bold text-muted-foreground">mm</span>
